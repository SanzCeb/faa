...
def codificar_datos(diccionarios, matrizDatos):
  """Funcion que codifica el conjunto de datos leidos del fichero"""
  matrizDatosTraspuesta = np.transpose(matrizDatos)
  codificadaTraspuesta = list(map(codificar_atributo,
                                  diccionarios,
                                  matrizDatosTraspuesta))
  return np.array(codificadaTraspuesta).transpose()
