    ...
    def __clasifica_dato (self,dato, atributosDiscretos, diccionario ):
        def __num_casos_a_posteriori ( hipotesis ):
            ...
            if prob_a_priori == 0 or 0 in verosimilitudes: 
                ...
                return math.log(fp.reduce(op.mul,verosimilitudes,prob_a_priori))
                
        conjunto_hipotesis = list(diccionario[-1].values())
        casos_favorables  = list(map( __num_casos_a_posteriori,conjunto_hipotesis))

        return conjunto_hipotesis[casos_favorables.index(max(casos_favorables))]
